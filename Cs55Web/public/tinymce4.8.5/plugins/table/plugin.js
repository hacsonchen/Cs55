(function() {
  var table = (function() {
    ...
    var unwrap = function(wrapper) {
      ...
      if (children$$1.length > 0) { before$1(wrapper, children$$1) }
      remove$2(wrapper)
    }
    ...
    var viewport = function(element) {
      ...
      if (!attached(element, html)) { return Position(0, 0) }
    ...
    var remove$3 = function(element, attr, id) {
      ...
      if (nu.length > 0) { set(element, attr, nu.join(' ')) } else { remove(element, attr) }
      return false
    }
    ...
    var add$2 = function(element, clazz) {
      if (supports(element)) { element.dom().classList.add(clazz) } else { add$1(element, clazz) }
    }
    ...
    var beforeSpecial = function(element, offset) {
      ...
      if (name$$1 === 'input') { return Situ.after(element) } else if (!contains([
        'br',
        'img'
      ], name$$1)) { return Situ.on(element, offset) } else { return offset === 0 ? Situ.before(element) : Situ.after(element) }
    }
    ...
    var getPartialBox = function(bridge, element, offset) {
      if (offset >= 0 && offset < getEnd(element)) { return bridge.getRangedRect(element, offset, element, offset + 1) } else if (offset > 0) { return bridge.getRangedRect(element, offset - 1, element, offset) }
      return Option.none()
    }
    ...
    var getBoxAt = function(bridge, element, offset) {
      if (isElement(element)) { return getElemBox(bridge, element).map(toCaret) } else if (isText(element)) { return getPartialBox(bridge, element, offset).map(toCaret) } else { return Option.none() }
    }
    var getEntireBox = function(bridge, element) {
      if (isElement(element)) { return getElemBox(bridge, element).map(toCaret) } else if (isText(element)) { return bridge.getRangedRect(element, 0, element, getEnd(element)).map(toCaret) } else { return Option.none() }
    }
    var Rectangles = {
      getBoxAt: getBoxAt,
    ...
    var adjustDown = function(bridge, element, guessBox, original, caret) {
      ...
      if (Math.abs(guessBox.bottom() - original.bottom()) < 1) { return adt$2.retry(lowerCaret) } else if (guessBox.top() > caret.bottom()) { return adt$2.retry(lowerCaret) } else if (guessBox.top() === caret.bottom()) { return adt$2.retry(Carets.moveDown(caret, 1)) } else { return inOutsideBlock(bridge, element, caret) ? adt$2.retry(Carets.translate(lowerCaret, JUMP_SIZE, 0)) : adt$2.none() }
    }
    ...
    var scan = function(bridge, isRoot, element, offset, direction, numRetries) {
      ...
      return tryCursor(bridge, isRoot, element, offset, direction).bind(function(situs) {
        ...
        return BeforeAfter.cata(analysis, function() {
          ...
          if (eq(element, cell) && offset === 0) { return tryAgain(bridge, element, offset, Carets.moveUp, direction) } else { return scan(bridge, isRoot, cell, 0, direction, numRetries - 1) }
        }, function(cell) {
          if (eq(element, cell) && offset === getEnd(cell)) { return tryAgain(bridge, element, offset, Carets.moveDown, direction) } else { return scan(bridge, isRoot, cell, getEnd(cell), direction, numRetries - 1) }
        })
      })
    }
    ...
    var keyboard = function(win, container, isRoot, annotations) {
      ...
      var keydown = function(event, start, soffset, finish, foffset, direction) {
        ...
        var handler = CellSelection.retrieve(container, annotations.selectedSelector()).fold(function() {
          ...
          if (SelectionKeys.isDown(keycode) && shiftKey) { return update([rc(+1, 0)]) } else if (SelectionKeys.isUp(keycode) && shiftKey) { return update([rc(-1, 0)]) } else if (direction.isBackward(keycode) && shiftKey) {
            return update([
              rc(0, -1),
              rc(-1, 0)
            ])
          ...
          } else if (SelectionKeys.isNavigation(keycode) && shiftKey === false) { return clearToNavigate } else { return Option.none }
        })
        return handler()
      }
      var keyup = function(event, start, soffset, finish, foffset) {
        return CellSelection.retrieve(container, annotations.selectedSelector()).fold(function() {
          ...
          if (SelectionKeys.isNavigation(keycode)) { return KeySelection.sync(container, isRoot, start, soffset, finish, foffset, annotations.selectRange) } else { return Option.none() }
        }, Option.none)
      }
      return {
        keydown: keydown,
