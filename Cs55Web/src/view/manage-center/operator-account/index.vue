<template>
  <div>
    <Card class="box-card el_card" :dis-hover="true">
      ...
      <br />
      <Table border ref="myTable" @on-selection-change="handleSelectionChange" :loading="tableLoading" :columns="columns" :data="dataList">
        <!-- 用户名称 -->
        ...
        <template slot="adminStatus" slot-scope="props">
          <i-switch v-model="props.row.adminStatus" size="small" @on-change="changeStatus(props.row.id,props.row.adminStatus)"></i-switch>
        </template>
        <!-- 操作 -->
        <template slot="action" slot-scope="props">
          ...
          &nbsp;|&nbsp;
          <a href="javascript:void(0)" @click="auth(props.row.id,props.row.adminName)">权限</a>
          <!-- <Button type="primary" size="small" @click="auth(props.row.id,props.row.adminName)">权限</Button> -->
          &nbsp;|&nbsp;
          <a href="javascript:void(0)" @click="delUserInfo(props.row.id)" class="deleteColor">删除</a>
          <!-- <Button type="error" size="small" @click="delUserInfo(props.row.id)">删除</Button> -->

        </template>
      </Table>
      <div style="text-align: right;margin-top:10px">
        <Page :total="pageTotal" :current="search.pageIndex" :page-size="search.pageSize" :page-size-opts=[10,20,50,100] @on-change="handlePage" @on-page-size-change="handlePageSize" show-elevator show-sizer show-total />
      </div>
    </Card>
    ...
    <Modal :title="detailTitle" v-model="modal1" width="850">
      <Form ref="ruleForm" :model="ruleForm" :rules="rules" :label-width="100">
        ...
        <Row>
          ...
          <Col :span="8">
          <FormItem label="创建时间：">
            {{$moment(ruleForm.creationTime).format('YYYY/MM/DD HH:mm:ss')}}
          </FormItem>
          </Col>
          <Col :span="8" v-if="ruleForm.id!=''">
          <FormItem label="最近登录：">
            {{ ruleForm.lastLoginTime ==='0001-01-01T00:00:00'?'': $moment(ruleForm.lastLoginTime).format('YYYY/MM/DD HH:mm:ss')}}
          </FormItem>
          </Col>
        </Row>
        <Row>
          ...
          <FormItem label="所属角色：" prop="roles">
            <Select :disabled="disable" v-model="ruleForm.roleIds" multiple filterable placeholder="请选择">
              <Option v-for="item in roleList" :key="item.value" :label="item.label" :value="item.value">
              </Option>
            </Select>
          </FormItem>
          </Col>
